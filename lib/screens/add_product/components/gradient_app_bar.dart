...
class GradientAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CubitBuilder<AddProductCubit, AddProductState>(
      ...
      builder: (context, state) {
        if (state is SetMainColor) {
          return buildAppBarWithPrimaryColor(statusbarHeight, context, state);
        } else {
          return buildAppBarWithDefaultColor(statusbarHeight, context);
        }
      },
    );
  }

  buildAppBarWithDefaultColor(double statusbarHeight, BuildContext context) {
  ...
  buildAppBarWithPrimaryColor(
      double statusbarHeight, BuildContext context, SetMainColor state) {
    return AnimatedContainer(
      duration: Duration(milliseconds: 500),
      padding: EdgeInsets.only(top: statusbarHeight),
      height: statusbarHeight + barHeight,
